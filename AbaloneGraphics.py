...
import random, sys, os, Abalone, variables
...
class Cell(ButtonBehavior, Image):
    def __init__(self, **kwargs):
        ...
        ButtonBehavior.__init__(self, **kwargs)
        Image.__init__(self, **kwargs)
        self.source = path + '/bl.png' # if random.random() > 0.5 else path + '/b.png'
        self.index = total_cell_counter
        total_cell_counter += 1

    def behave(self, touch):
        if self.source == path + '/w.png' and allow_choosing_white:
            self.source = path + '/wm.png'
        elif self.source == path + '/b.png' and allow_choosing_black:
            self.source = path + '/bm.png'
        elif self.source == path + '/bm.png':
            self.source = path + '/b.png'
        elif self.source == path + '/wm.png':
            self.source = path + '/w.png'
        else:
            return
...
class Background(FloatLayout):
    def __init__(self, **kwargs):
        ...
        background_texture = Button(background_color = (1, 2, 200, 1)) #source = path + '/a.png', allow_stretch = 1, keep_ratio = 0)
        self.add_widget(background_texture)
        b = SpecialButton(disabled = False)
        b.source = path + '/board.png'
        b.allow_stretch = 1
    ...
    def behave(self, touch, *args):
        ...
        if game_state == 3: #handle both inputs and make move
            ...
            if not len(indices) == 0:
                self.children[1].header.text = 'Computer Playing' if (not disable_ai and MOVES % 2 == 0) else 'Choose Balls ' + ('(White)' if MOVES % 2 == 0 else '(Black)')
            if type(touch) == type(''):
                where = touch
            self.update_board()
    ...
    def update_board(self):
        ...
        for i in range(len(a.board)):
            self.children[1].board[i].source = path + '/w.png' if a.board[i].value == 'white'\
                else path + '/b.png' if a.board[i].value == 'black' else path + '/bl.png'
        for i in range(6):
            containers[0][i].source = path + ('/w.png' if a.pushed_out['white'] >= i + 1 else '/bl.png')
            containers[1][i].source = path + ('/b.png' if a.pushed_out['black'] >= i + 1 else '/bl.png')
...
class Board(BoxLayout):
    ...
    def __init__(self, **kwargs):
        ...
        self.header = Label(color = (255, 255, 255, 1), text = 'Press to Begin the Game!',
                            font_name = 'header_font.otf', font_size = '45sp')
        ...
        for j in range(3):
            for i in reversed(range(3)):
                if i != 1:
                    ...
                    b.source = path + '/arrow_off' + str(b.value) + '.png'
...
class SelectionButton(SpecialButton):
    def __init__(self, **kwargs):
        self.value = 0
        self.direction = ''
        SpecialButton.__init__(self, **kwargs)
        self.always_release = True

    def on_press(self):
        self.source = path + '/arrow_on' + str(self.value) + '.png'

    def on_release(self):
        self.source = path + '/arrow_off' + str(self.value) + '.png'
...
class GameScreen(Screen):
    ...
    def reset(self, *args):
        global game_state, containers, MOVES, allow_choosing_white, percentage_display,\
...
class HomeScreen(Screen):
    def __init__(self, **kwargs):
        ...
        base = FloatLayout()
        background = Image(source = path + '/background.png', allow_stretch = True, keep_ratio = False)
        cover = BoxLayout()
        ...
        for i in range(4):
            ...
            for j in range(5):
                if not ((i == 1 and j in [1, 2, 3]) or (i == 2 and j == 2) or (i == 0 and j in [1,2,3])):
                    ...
                    if i == 1 and j == 1:
                        b = SpecialButton(source = path + '/single_player.png', size_hint=(0.5, 1)) #add single player button
                        b.bind(on_press = self.single_player)
                        a.add_widget(b)
                        self.selection.append(b)
                    if i == 1 and j == 2:
                        b = SpecialButton(source = path + '/2 players.png', size_hint=(0.5, 1)) #add the 2 players button
                        b.bind(on_press = self.two_players)
                        a.add_widget(b)
                        self.selection.append(b)
                    if i == 1 and j == 3:
                        b = SpecialButton(source = path + '/rules.png', size_hint=(0.5, 1)) #add the rules button
                        b.bind(on_press = self.rules)
                        a.add_widget(b)
                        self.selection.append(b)
                    if i == 2 and j == 2:
                        b = SpecialButton(source = path + '/classic layout.png', size_hint=(0.5, 1)) #add the layout button
                        b.id = 'Regular Layout'
                        b.bind(on_press = self.layout)
                        a.add_widget(b)
                    if i == 0 and j == 2:
                        b = Image(source = path + '/title.png', size_hint=(0.5, 1)) #add the title image
                        a.add_widget(b)
    ...
    def reset(self, *args):
        ...
        if self.to_be_reset:
            ...
            self.selection[0].source = path + '/single_player.png'
            self.selection[0].bind(on_press = self.single_player)
            self.selection[1].source = path + '/2 players.png'
            self.selection[1].bind(on_press = self.two_players)
            self.selection[2].source = path + '/rules.png'
            self.selection[2].bind(on_press = self.rules)

    def layout(self, touch):
        '''Handle layout button'''
        global regular_layout
        if touch.id == 'Daisy Layout':
            touch.id = 'Regular Layout'
            touch.source = path + '/classic layout.png'
            variables.regular_layout = True
        else:
            touch.id = 'Daisy Layout'
            touch.source = path + '/daisy layout.png'
    ...
    def single_player(self, touch):
        ...
        for i in range(len(self.selection)):
            self.selection[i].source = path + '/' + ['Easy', 'Medium', 'Hard'][i] + '.png'
            self.selection[i].id = str(int(i+1))
...
class RulesScreen(Screen):
    def __init__(self, **kwargs):
        ...
        Screen.__init__(self, **kwargs)
        base = BoxLayout()
        b = SpecialButton(source = path + '/rules1.png', allow_stretch = True, keep_ratio = False, id = '1')
    ...
    def continue_rules(self, touch):
        ...
        if touch.id == '4':
            ...
            touch.id = str(int(touch.id) + 1)
        touch.source = path + '/rules' + touch.id + '.png'
...
class EndScreen(Screen):
    ...
    def run(self, *args):
        ...
        background_image = Image(source = path + '/end_screen_image.png', allow_stretch = 1, keep_ratio = 0)
        self.add_widget(background_image)
        b = Label(text='Thanks for playing!', font_name='font.ttf', font_size='140sp', color = (205/255, 87/255, 9/255, 1))
    ...
    def end_screen_popup(self, *args):
        ...
        self.popup.children[0].add_widget(SpecialButton(source = path + '/new game.png', on_press = self.restart))
        self.popup.children[0].add_widget(SpecialButton(source = path + '/quit game.png', on_press = self.quit))
        self.popup.children[0].add_widget(Button(text = 'No, I\'d prefer to play cookie clicker instead',
                                                 color = (0, 0, 0, 1), background_color = (255, 255, 255, 0),
                                                 on_press = self.cookie_clicker, id = '0'))
        self.popup.background = path + '/popup.png'
        self.popup.open()
